...
public class ConcatenatedWords {
    ...
    public static void main(String[] args) {
        ...
        String[] words = {"a","b","c", "abcd"};
        new ConcatenatedWordsSolution().findAllConcatenatedWordsInADict(words);
...
class ConcatenatedWordsSolution {
    TrieArray root;
    public List<String> findAllConcatenatedWordsInADict(String[] words) {
       ...
       for(String word : words){
           ...
           for(char c : word.toCharArray()){
               if(current.child[c-'a']==null)
                   current.child[c-'a'] = new TrieArray();
       ...
       for(String word : words){
           if(word.equalsIgnoreCase("abcd")) {
               System.out.println(" ------------------------- ");
               System.out.println("next word --> " + word);
               dfs(word, root, results, 0, 0);
           }
       }
    ...
    public void dfs(String word, TrieArray node, List<String> results, int index, int splitCount){
        ...
        if(index==word.length()) {
            ...
            if(splitCount>1 && !results.contains(word))
                results.add(word);
            return;
        }
        if(node.wordEnd){
            System.out.println(" calling root dfs ");
            dfs(word, root.child[word.charAt(index)-'a'], results, index+1, splitCount+1);
        }
        System.out.println(" calling direct dfs ");
        dfs(word, node.child[word.charAt(index)-'a'], results, index+1, splitCount);
